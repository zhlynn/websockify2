...
void ws_config_usage(const char *progname) {
    fprintf(stderr,
        "websockify2 (v%s) — High-Performance WebSocket-to-TCP proxy\n"
        "Visit https://github.com/zhlynn/websockify2 for more information.\n\n"
...
int ws_config_parse(ws_config_t *cfg, int argc, char **argv) {
    ...
    while ((opt = getopt_long(argc, argv,
                              "w:c:k:C:st:Hr:Dp:l:L:i:T:U:u:vhV",
                              long_opts, NULL)) != -1) {
        switch (opt) {
    ...
    /* Validation: missing required positional args → show usage. */
    if ((cfg->listen_port <= 0 && cfg->unix_listen[0] == '\0') ||
        (cfg->target_port <= 0 && cfg->unix_target[0] == '\0' &&
         cfg->token_plugin[0] == '\0')) {
        ws_config_usage(argv[0]);
        return 1;
    }
