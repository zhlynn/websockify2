...
static ws_conn_t *conn_alloc(ws_proxy_ctx_t *ctx) {
    ...
    ws_conn_t *conn = ctx->free_list;
    ctx->free_list = conn->next;
...
int ws_proxy_init(ws_proxy_ctx_t *ctx, ws_config_t *config,
    ...
    ctx->max_conn = config->max_connections;

    /* Pre-allocate connection pool */
    ctx->conn_pool = (ws_conn_t *)calloc((size_t)ctx->max_conn, sizeof(ws_conn_t));
    if (!ctx->conn_pool) return -1;

    /* Build free list */
    ctx->free_list = &ctx->conn_pool[0];
    for (int i = 0; i < ctx->max_conn - 1; i++) {
        ctx->conn_pool[i].next = &ctx->conn_pool[i + 1];
        ctx->conn_pool[i].client_fd = WS_INVALID_SOCKET;
        ctx->conn_pool[i].target_fd = WS_INVALID_SOCKET;
    }
    ctx->conn_pool[ctx->max_conn - 1].next = NULL;
    ctx->conn_pool[ctx->max_conn - 1].client_fd = WS_INVALID_SOCKET;
    ctx->conn_pool[ctx->max_conn - 1].target_fd = WS_INVALID_SOCKET;
...
void ws_proxy_on_accept(ws_event_loop_t *loop, ws_socket_t fd, int events, void *data) {
    ...
    for (;;) {
        ...
        if (!conn) {
            ws_log_warn("connection pool exhausted, rejecting %s:%d", addr, port);
...
static ws_proxy_ctx_t *conn_get_ctx(ws_conn_t *conn) {
    /* Walk back to find which pool this connection belongs to.
     * The proxy context is stored such that conn_pool <= conn < conn_pool + max_conn */
    /* We store ctx pointer in a simpler way — via the event loop's listen fd data.
     * For now, use a global. This is per-worker so it's safe. */
...
static int process_ws_frame(ws_conn_t *conn) {
    for (;;) {
        ...
        while (chunk > 0) {
            ...
            case WS_OP_CONT:
                /* Forward to target */
                ws_buf_write(&conn->target_send, tmp, rd);
                if (conn->record)
    ...
    if (events & WS_EV_READ) {
        ...
        for (;;) {
            ...
            if (conn->state == CONN_STATE_HTTP_REQUEST) {
                ...
                ws_buf_write(&conn->client_recv, tmp, (uint32_t)n);
            }
        }
...
static void on_target_event(ws_event_loop_t *loop, ws_socket_t fd, int events, void *data) {
    ...
    if (events & WS_EV_READ) {
        ...
        for (;;) {
            ...
            ws_buf_write(&conn->target_recv, tmp, (uint32_t)n);
        }
