...
if $WEBSOCKIFY 2>/dev/null; then
    assert_pass 1 "rejects missing arguments"
else
    assert_pass 0 "rejects missing arguments"
fi
...
$WEBSOCKIFY --max-connections 100 --buffer-size 32768 \
            --keepalive-idle 30 --keepalive-intvl 5 --keepalive-cnt 3 \
            $PORT_WS localhost:$PORT_ECHO &
WS_PID=$!
sleep 0.5
