...
static int test_config_defaults(void) {
    ws_config_t cfg;
    ws_config_init(&cfg);
    ASSERT_EQ(cfg.max_connections, WS_DEFAULT_MAX_CONN);
    ASSERT_EQ(cfg.buffer_size, WS_DEFAULT_BUFSIZE);
...
static int test_config_parse_tuning(void) {
    ws_config_t cfg;
    ws_config_init(&cfg);
    char *argv[] = {"ws", "--max-connections", "50000",
                    "--buffer-size", "131072",
                    "--idle-timeout", "300",
                    "8080", "localhost:5900", NULL};
    reset_getopt();
    ASSERT_EQ(ws_config_parse(&cfg, 9, argv), 0);
    ASSERT_EQ(cfg.max_connections, 50000);
...
static int test_config_missing_target(void) {
    ...
    fclose(stderr);
    stderr = old;
    ASSERT_EQ(ret, -1);  /* no target and no token-plugin */
    return 0;
}
